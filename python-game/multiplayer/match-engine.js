...
if (!ROOM_CODE) {
  ...
  throw new Error('No room code — redirecting');
}
...
function applyLevelConfig(levelNum) {
  ...
  me.robot = { ...START };
  opp.robot = { ...START };
  // 🔑 อัปเดต camTarget ให้อยู่กลาง grid ของด่านนี้
  camTarget.set(GRID*CELL/2 - CELL/2, 0, GRID*CELL/2 - CELL/2);
  // ปรับ camDist ตามขนาด grid
  camDist = Math.max(14, GRID*2.5);
}
...
// 🔑 ใช้ค่า default ก่อน — applyLevelConfig จะ update ค่าจริงให้
const camTarget = new THREE.Vector3(4, 0, 4);
...
// --- Touch (mobile) ---
let _tLastX=0, _tLastY=0, _pinchStart=0, _pinchCam=14;
function _touchDist(t1, t2) {
  const dx=t1.clientX-t2.clientX, dy=t1.clientY-t2.clientY;
  return Math.sqrt(dx*dx+dy*dy);
}
canvas.addEventListener('touchstart', e => {
  if(e.touches.length===1){ _tLastX=e.touches[0].clientX; _tLastY=e.touches[0].clientY; }
  else if(e.touches.length===2){ _pinchStart=_touchDist(e.touches[0],e.touches[1]); _pinchCam=camDist; }
}, { passive: true });
canvas.addEventListener('touchmove', e => {
  if(e.touches.length===1){
    const dx=e.touches[0].clientX-_tLastX, dy=e.touches[0].clientY-_tLastY;
    camAngle += dx*0.012;
    camHeight = Math.max(4, Math.min(20, camHeight - dy*0.04));
    _tLastX=e.touches[0].clientX; _tLastY=e.touches[0].clientY;
    updateCamera();
    e.preventDefault();
  } else if(e.touches.length===2){
    const newDist = _touchDist(e.touches[0], e.touches[1]);
    camDist = Math.max(8, Math.min(22, _pinchCam * (_pinchStart/newDist)));
    updateCamera();
    e.preventDefault();
  }
}, { passive: false });
...
async function buildRobots(){
  // me
  const myPlayer = players.find(p => String(p.user_id) === String(PY3D.user.id));
  ...
  // opponent
  const oppPlayer = players.find(p => String(p.user_id) !== String(PY3D.user.id));
  if (oppPlayer) {
...
function syncPickCoin(){
  ...
  return queueAnim(async () => {
    ...
    await PY3D.sb.from('py3d_room_players')
      ...
      .eq('room_code', ROOM_CODE).eq('user_id', String(PY3D.user.id));
...
async function applyOppPick(coinIdx){
  // ไม่ increment opp.coins ที่นี่ — postgres handler รับผิดชอบ
  // (ป้องกัน race condition double-count กับ realtime UPDATE)
...
async function handleRoomFinished(newRoom){
  if(finished) return;
  finished = true;
  const won = String(newRoom.winner_id) === String(PY3D.user.id);
  if(room.mode === 'race'){
    ...
    // golf
    const { data } = await PY3D.sb.from('py3d_room_players').select('*').eq('room_code', ROOM_CODE);
    const myId = String(PY3D.user.id);
    const myRow  = data.find(p => String(p.user_id) === myId) || {};
    const oppRow = data.find(p => String(p.user_id) !== myId) || {};
...
async function subscribe(){
  ...
  channel.on('broadcast', { event: 'state' }, ({ payload }) => {
    if(String(payload.from) === String(PY3D.user.id)) return;
  ...
  channel.on('postgres_changes', {
    ...
    if(String(np.user_id) !== String(PY3D.user.id)){
      opp.coins = np.coins;
      updateHud();
      if(np.finished_at){
