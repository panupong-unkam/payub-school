...
function renderPlayers() {
  ...
  const others = players.filter(p => String(p.user_id) !== myId);
  const mePlayer = players.find(p => String(p.user_id) === myId);
...
async function toggleReady() {
  ...
  const { error } = await PY3D.sb.from('py3d_room_players')
    ...
    .eq('room_code', ROOM_CODE).eq('user_id', String(me.id));
  btn.disabled = false;
  if (error) {
...
async function leaveRoom() {
  ...
  await PY3D.sb.from('py3d_room_players')
    .delete().eq('room_code', ROOM_CODE).eq('user_id', String(me.id));
  // if i was host, delete room (cascade kills players)
  if (room && String(room.host_id) === String(me.id)) {
...
async function startCountdown() {
  ...
  const isHost = room && String(room.host_id) === String(PY3D.user.id);
  ...
  if (!countdownStarted) {
    ...
    if (PY3D.user) {
      PY3D.sb.from('py3d_room_players')
        .delete().eq('room_code', ROOM_CODE).eq('user_id', String(PY3D.user.id));
    }
  }
});
