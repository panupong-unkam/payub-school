...
`,
  ...
  8: {
    ...
    walls: [
      // ลบ (3,1) และ (3,5) ออก เพื่อให้เข้าถึง coin กลาง (3,3) ได้
...
PY3D.isLevelUnlocked = function (levelNum) {
  ...
  // 🔓 Teacher bypass — ครูเข้าได้ทุกด่าน (ใช้สำหรับสอน/ทดสอบ)
  try {
    const u = PY3D.user || JSON.parse(localStorage.getItem('payub_user') || 'null');
...
// Helper: ใช้ใน UI เพื่อรู้ว่าใครเป็นครู
PY3D.isTeacher = function () {
  try {
    const u = PY3D.user || JSON.parse(localStorage.getItem('payub_user') || 'null');
