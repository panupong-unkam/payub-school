...
function navigate(page, el, isShortcut = false) {
    ...
    if (!targetPage) {
        console.warn('[navigate] page-' + page + ' not found');
        return;
    }
...
async function loadData() {
   ...
   // ⭐ 1. กรองวิชาและใบงาน
    ...
    const subjectsHTML = displaySubs.length ? displaySubs.map(s => {
        const safeName = s.name || 'วิชาไม่ระบุชื่อ';
        let icon = s.icon || (safeName.includes('คำนวณ') ? '🧠' : (safeName.includes('หุ่นยนต์') ? '🤖' : '💻'));
        s.name = safeName;
...
async function updateMobileGradeSummary() {
    ...
    if (myScores && myScores.length > 0) {
        ...
        itemsList.innerHTML = myScores.map(s => {
            if (!s.assignments) return '';  // กัน FK เสีย
            totalReceived += (s.score || 0);
            totalPossible += (s.assignments.total_score || 0);
            return `
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span style="color: var(--text-muted); text-overflow: ellipsis; white-space: nowrap; overflow: hidden; max-width: 180px;">• ${s.assignments.title || 'ใบงาน'}</span>
...
function initMascotAutoHide() {
    // 🦉 mascot อยู่ใน fab-menu แล้ว — ไม่ต้องซ่อนอัตโนมัติ
    // (เมนูจะซ่อน-เปิดจากปุ่ม + อยู่แล้ว)
    return;
}
